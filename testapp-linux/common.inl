...
template <typename T> inline volatile T& reg(regaddr_t base,int offset){
...
inline void csr_write8(regaddr_t baseaddr,uint8_t value){
    reg<uint8_t>(baseaddr,0x00)=value;
}
inline uint8_t csr_read8(regaddr_t baseaddr){
    uint8_t rval = reg<uint8_t>(baseaddr,0x00);
    return rval;
}
inline uint32_t csr_read32(regaddr_t baseaddr){
...
inline uint64_t csr_read64(regaddr_t baseaddr){
    return reg<uint64_t>(baseaddr,0x00);
}

////////////////////////////////////////////////////////////////

void inline csr_write32(regaddr_t baseaddr,uint32_t value){
...
void inline csr_write64(regaddr_t baseaddr,uint64_t value){
...
inline regaddr_t mapregbank(uint32_t physaddr, uint32_t physlen){
  int fd = open("/dev/mem", O_RDWR|O_SYNC);
  size_t pagesize = sysconf(_SC_PAGE_SIZE);
  off_t page_base = (physaddr / pagesize) * pagesize;
  off_t page_offset = physaddr - page_base;
  auto mapped = (regaddr_t) mmap( NULL,
                                 page_offset + physlen,
  ...
  printf("mapping physaddr<%08x> len<%08x> to virtaddr<%px>\n",physaddr,physlen,mapped);
  close(fd);
  return mapped;
}
