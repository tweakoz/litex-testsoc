...
#define LINUX_IMAGE_BASE 0x40000000
#define LINUX_DTB_BASE   0x40FF0000
...
static void vexriscv_machine_mode_init(void) {
	...
	csr_write(medeleg, MEDELEG_INSTRUCTION_PAGE_FAULT |
...
static void vexriscv_machine_mode_trap_to_supervisor_trap(uint32_t _sepc, uint32_t _mstatus){
	csr_write(mtvec,    vexriscv_machine_mode_trap_entry);
	csr_write(sbadaddr, csr_read(mbadaddr));
	csr_write(scause,   csr_read(mcause));
	csr_write(sepc,     _sepc);
	csr_write(mepc,	    csr_read(stvec));
	csr_write(mstatus,
			  (_mstatus & ~(MSTATUS_SPP | MSTATUS_MPP | MSTATUS_SIE | MSTATUS_SPIE))
			| ((_mstatus >> 3) & MSTATUS_SPP)
			| (0x0800 | MSTATUS_MPIE)
			| ((_mstatus & MSTATUS_SIE) << 4)
	);
}
...
void vexriscv_machine_mode_trap(void) {
	...
	/* Interrupt */
	if(cause&0x80000000){
        uint32_t irqid = cause & 0xff;
        if( irqid>=16 ) { // local
            counter++;
        }
        else {
    		switch(irqid){
                case CAUSE_MACHINE_EXT_IRQ: {
                    counter++;
                    csr_set(sip, MIP_SEIP); // supervisor-level timer interrupt
                    csr_clear(mie, MIE_MEIE); //  Machine Timer Interrupt Enable
    			...
    			case CAUSE_MACHINE_TIMER: {
    				csr_set(sip, MIP_STIP); // supervisor-level timer interrupt
    				csr_clear(mie, MIE_MTIE); //  Machine Timer Interrupt Enable
    			} break;
	...
	/* Exception */
		...
		switch(cause){
			...
			case CAUSE_SCALL:{
				...
				switch(which){
					case SBI_CONSOLE_PUTCHAR: { // hvc tty (hypervisor terminal)
						litex_putchar(a0);
						csr_write(mepc, csr_read(mepc) + 4);
					} break;
					case SBI_CONSOLE_GETCHAR: { // hvc tty (hypervisor terminal)
